...
def LAFs2ellT(LAFs):
    ...
    if LAFs.is_cuda:
        ...
        ellipses = ellipses.to(LAFs.device)
...
def ells2LAFsT(ells):
    ...
    LAFs[:, 0:2, 0:2] = rectifyAffineTransformationUpIsUp(A / sc.view(-1, 1, 1).repeat(1, 2, 2)) * sc.view(-1, 1,
                                                                                                           1).repeat(1,
                                                                                                                     2,
                                                                                                                     2)
    return LAFs
...
def checkTouchBoundary(LAFs):
    ...
    if LAFs.is_cuda:
        # pts = pts.cuda()
        pts = pts.to(LAFs.device)
    out_pts = torch.bmm(LAFs_to_H_frames(LAFs), pts.expand(LAFs.size(0), 3, 4))[:, :2, :]
    good_points = 1 - (((out_pts > 1.0).float() + (out_pts < 0.0).float()).sum(dim=1).sum(dim=1) > 0).float()
    return good_points
...
def bsvd2x2(As):
    ...
    if As.is_cuda:
        ...
        SIG = torch.zeros(As.size(0), 2, 2).to(As.device)
...
def LAF2center(LAF):
    x = [0]
    # x.extend(list(np.sin(np.linspace(0, 2 * np.pi, 10))))
    x = np.array(x).reshape(1, -1)
    y = [0]
    # y.extend(list(np.cos(np.linspace(0, 2 * np.pi, 10))))
    y = np.array(y).reshape(1, -1)
    HLAF = np.concatenate([LAF, np.array([0, 0, 1]).reshape(1, 3)])
    H_pts = np.concatenate([x, y, np.ones(x.shape)])
    H_pts_out = np.transpose(np.matmul(HLAF, H_pts))
...
def visualize_LAFs_center(img, LAFs, color='r', show=False, save_to=None):
    work_LAFs = convertLAFs_to_A23format(LAFs)
    plt.figure()
    plt.imshow(img)
    # plt.imshow(255 - img)
    for i in range(len(work_LAFs)):
        center = LAF2center(work_LAFs[i, :, :])
        plt.scatter(center[:, 0], center[:, 1], s=10, c='b', marker='.')
    if show:
        plt.show()
    if save_to is not None:
        plt.savefig(save_to)
...
def rectifyAffineTransformationUpIsUp(A):
    ...
    A2_ell = torch.cat(
        [((A[:, 1, 1] * A[:, 0, 1] + A[:, 1, 0] * A[:, 0, 0]) / (b2a2 * det)).contiguous().view(-1, 1, 1),
...
def rectifyAffineTransformationUpIsUpFullyConv(A):  # A is (n,4,h,w) tensor
    ...
    return torch.cat([(b2a2 / det).contiguous(), 0 * det.contiguous(),
                      (A[:, 3:4, :, :] * A[:, 1:2, :, :] + A[:, 2:3, :, :] * A[:, 0:1, :, :]) / (b2a2 * det),
                      (det / b2a2).contiguous()], dim=1)
        ...
        for batch_idx in range(n_batches):
            ...
            if batch_idx == 0:
                if img.size(0) != grid.size(0):
                    first_batch_out = F.grid_sample(img.expand(end - st, img.size(1), img.size(2), img.size(3)),
                                                    grid[st:end, :, :, :])  # kwargs)
                else:
                ...
                if img.size(0) != grid.size(0):
                    out[st:end, :, :] = F.grid_sample(img.expand(end - st, img.size(1), img.size(2), img.size(3)),
                                                      grid[st:end, :, :, :])
                else:
...
def extract_patches_from_pyramid_with_inv_index(scale_pyramid, pyr_inv_idxs, LAFs, PS=19):
    ...
    if LAFs.is_cuda:
        ...
        patches = patches.to(LAFs.device)
...
def sc_y_x2LAFs(sc_y_x):
    ...
    if sc_y_x.is_cuda:
        ...
        base_LAF = base_LAF.to(sc_y_x.device)
...
def get_pyramid_and_level_index_for_LAFs(dLAFs, sigmas, pix_dists, PS):
    ...
    if dLAFs.is_cuda:
        ...
        closest_imgs = closest_imgs.to(dLAFs.device)
        oct_idxs_full = oct_idxs_full.to(dLAFs.device)
        level_idxs_full = level_idxs_full.to(dLAFs.device)
...
def LAF2mainSA(LAF):
    ...
    angle1 = np.arctan2(point_center[(2, 4), 1] - point_center[(1, 3), 1],
                        point_center[(2, 4), 0] - point_center[(1, 3), 0]) * 180 / np.pi
    angle1[angle1 < 0] = angle1[angle1 < 0] + 360

    HLAF = np.concatenate([LAF, np.array([0, 0, 1]).reshape(1, 3)])
    warped_center = np.transpose(np.matmul(HLAF, point_center.transpose((1,0))))
    warped_center[:, 0] = warped_center[:, 0] / warped_center[:, 2]
    warped_center[:, 1] = warped_center[:, 1] / warped_center[:, 2]

    # 计算变换后十字点的位置间距
    end_point = warped_center[1:]
    diff = end_point - warped_center[0:1]
    # 若十字的拓扑形状改变,说明该位置无法用单应变换来投影,则直接保持原始的十字形状
    ...
    angle2 = np.arctan2(warped_center[(2, 4), 1] - warped_center[(1, 3), 1],
                        warped_center[(2, 4), 0] - warped_center[(1, 3), 0]) * 180 / np.pi
    ...
    angle_rela = angle_rela * np.pi / 180

    return scale_rela, angle_rela
