...
from AffNet.LAF import LAFs2ell, abc2A, angles2A, generate_patch_grid_from_normalized_LAFs, extract_patches, \
    get_inverted_pyr_index, denormalizeLAFs, extract_patches_from_pyramid_with_inv_index, \
...
from AffNet.HandCraftedModules import HessianResp, AffineShapeEstimator, OrientationDetector, ScalePyramid, \
    NMS3dAndComposeA
import time
...
class ScaleSpaceAffinePatchExtractor(nn.Module):
        ...
        for oct_idx in range(len(self.sigmas)):
            ...
            for level_idx in range(1, len(octave) - 1):
                ...
                if x.is_cuda:
                    # pyr_id = pyr_id.cuda()
                    # lev_id = lev_id.cuda()
                    pyr_id = pyr_id.to(x.device)
                    lev_id = lev_id.to(x.device)
    ...
    def getAffineShape(self, final_resp, LAFs, final_pyr_idxs, final_level_idxs,
                       num_features=0, scale_list=None):
        pe_time = 0
        affnet_time = 0
        pyr_inv_idxs = get_inverted_pyr_index(self.scale_pyr, final_pyr_idxs, final_level_idxs)
        t = time.time()
        patches_small = extract_patches_from_pyramid_with_inv_index(self.scale_pyr, pyr_inv_idxs, LAFs,
                                                                    PS=self.AffNet.PS)
        pe_time += time.time() - t
        t = time.time()
        ...
        if final_resp.is_cuda:
            # base_A = base_A.cuda()
            base_A = base_A.to(final_resp.device)
        ...
        for i in range(self.num_Baum_iters):
            ...
            if i != self.num_Baum_iters - 1:
                pe_time += time.time() - t
                t = time.time()
                patches_small = extract_patches_from_pyramid_with_inv_index(self.scale_pyr, pyr_inv_idxs, new_LAFs,
                                                                            PS=self.AffNet.PS)
                pe_time += time.time() - t
        ...
        idxs_mask = ((ratio < 6.0).float() * (ratio > (1. / 6.)).float()) * checkTouchBoundary(new_LAFs)
        ...
        # ratio[idxs]可以作为scale
        scale = ratio[idxs]
        if scale_list is not None:
            scale_list.append(scale)
        return final_resp, new_LAFs, final_pyr_idxs, final_level_idxs

    def getOrientation(self, LAFs, final_pyr_idxs, final_level_idxs, angle_list=None):
        pyr_inv_idxs = get_inverted_pyr_index(self.scale_pyr, final_pyr_idxs, final_level_idxs)
        patches_small = extract_patches_from_pyramid_with_inv_index(self.scale_pyr, pyr_inv_idxs, LAFs,
                                                                    PS=self.OriNet.PS)
        max_iters = 1
    ...
    def extract_patches_from_pyr(self, dLAFs, PS=41):
        ...
        patches = extract_patches_from_pyramid_with_inv_index(self.scale_pyr,
                                                              pyr_inv_idxs,
                                                              normalizeLAFs(dLAFs, self.scale_pyr[0][0].size(3),
                                                                            self.scale_pyr[0][0].size(2)),
                                                              PS=PS)
        return patches

    def forward(self, x, do_ori=False, scale_list=None, angle_list=None):
        ### Detection
        t = time.time()
        ...
        if self.num_Baum_iters > 0:
            responses, LAFs, final_pyr_idxs, final_level_idxs = self.getAffineShape(
                responses, LAFs, final_pyr_idxs, final_level_idxs, self.num, scale_list)
        # print(time.time() - t, 'affine shape iters')
        t = time.time()
        if do_ori:
            LAFs = self.getOrientation(LAFs, final_pyr_idxs, final_level_idxs, angle_list)
            # pyr_inv_idxs = get_inverted_pyr_index(self.scale_pyr, final_pyr_idxs, final_level_idxs)
